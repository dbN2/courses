...
int binary_to_decimal(string s){
    int sum = 0;  //resulting decimal number
    int num = stoi(s);  //convert binary string to int 
    int base = 1; 

    int temp = num;
    while (temp)
    {
        int last_digit = temp % 10;  //get last digit of binary number
        temp /= 10;  

        sum += last_digit * base;  //add digit^power to sum
        base *= 2;  //increase power and move to next digit
    }
...
string decimal_to_binary(int n) {
    ...
    for (int i = 0; i < bin.length() / 2; i++)   //reverse the string to obtain binary string
    {
        char temp = bin[i];
...
string add_binaries(string b1, string b2){
    ...
    while (i >= 0 || j >= 0 || s == 1)
    {
        ...
        bin = char(s % 2 + '0') + bin; //if digit sum is 1 or 3, add 1 to result
